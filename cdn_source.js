...
function intlTelSetGeoCode(form, code) {
  form.querySelectorAll("input[name=country]").forEach((el) => {
    el.value = code;
  });
}

function intlTelSetPhoneCode(form, code) {
  form.querySelectorAll(".phonecc").forEach((el) => {
    el.value = code;
  });
}

function initPhoneInput(form) {
  ...
  const iti = window.intlTelInput(phoneInput, {
    ...
    geoIpLookup: (callback) => {
      fetch("https://ipinfo.io", {
        ...
        .then((res) => res.json())
        .then((data) => {
          callback(data?.country || "cz");
        });
    },
  });

  phoneInput.addEventListener("countrychange", () => {
    const geo = iti.getSelectedCountryData();
    intlTelSetPhoneCode(form, geo.dialCode);
    intlTelSetGeoCode(form, geo.iso2);
    validatePhone(form, phoneInput, iti);
  });
...
function validatePhone(form, input, iti) {
...
function validateNameInput(form, input, errorSelector, fieldName = "Name") {
  const value = input.value.trim();
  const errorSpan = form.querySelector(errorSelector);
  const isValid = /^[A-Za-zА-Яа-яЁё\s'-]{3,}$/.test(value);
...
function validateEmailInput(form, input, errorSelector) {
  const value = input.value.trim();
  const errorSpan = form.querySelector(errorSelector);
...
function checkAllInputsValid(form) {
...
document.addEventListener("DOMContentLoaded", () => {
  document.querySelectorAll(".cdn-form").forEach((form) => {
    ...
    const phoneIti = initPhoneInput(form);

    if (nameInput) {
      nameInput.addEventListener("input", () => {
        validateNameInput(form, nameInput, ".first-name-error", "Name");
      });
    }

    if (lastNameInput) {
      lastNameInput.addEventListener("input", () => {
        validateNameInput(form, lastNameInput, ".last-name-error", "Last name");
      });
    }

    if (emailInput) {
      emailInput.addEventListener("input", () => {
        validateEmailInput(form, emailInput, ".email-error");
      });
    }

    if (phoneInput) {
      phoneInput.addEventListener("input", () => {
        validatePhone(form, phoneInput, phoneIti);
      });
    }
  });
});
...
document
  ...
  .forEach((e) =>
    e.addEventListener("submit", () =>
      document
        .querySelector(".overlay-yYhkgJucat")
        .classList.add("visible-yYhkgJucat")
    )
  );
...
document.querySelectorAll(".date-stamp").forEach((item) => {
  const formatType = item.dataset.format || "text";
  const offset = parseInt(item.dataset.offset || "0", 10);
  const locale = item.dataset.locale || "en-ES";
  ...
  item.textContent = date.toLocaleDateString(locale, options);
});